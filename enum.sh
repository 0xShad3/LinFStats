...
most_files() {
    ...
    find ${1} -maxdepth ${2} -type d | while read -r dir; do
        NUM=$(ls "$dir" -p | grep -v / | wc -l)
        [ -d "$dir" ] && printf "\n\t\t${GRE}Files: ${NCL}${YEL} $NUM %s:\t" "$dir"
    done | sort -k2nr | head -5
}

file_size() {
    printf "\n\n\t\t\t\t\t${RED}Directories with biggest size!${NCL}\n\n"
    printf "\n\t\t\t${BLU}===================================================================${NCL}\n\n"

    find ${1} -maxdepth ${2} -type d | while read -r dir; do
        NUM=$(du -h -d 1 "$dir")
        [ -d "$dir" ] && printf "\n\t\t${GRE}Size: ${NCL}${YEL} $NUM %s:\t ${NCL}" "$dir"
...
lastly_created
most_files "${ABS_PATH}" "${DEPTH}"
file_size "${ABS_PATH}" "${DEPTH}"
system_info
